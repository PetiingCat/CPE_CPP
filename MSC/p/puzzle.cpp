...
int equal_single_check(char a1[][3], char a2[][3]){
    int i,j,flag=1;
    for(i=0;i<3;i++){
        for(j=0;j<3;j++){
            if((a1[i][j])!=a2[i][j]){
...
int Transform_90(char P[][3], char A[][3], char AT[][3]){
    ...
    int i,j,n=3,dst=n-1,flag;
    for(i=0;i<n;i++,dst--){
        for(j=0;j<n;j++){
            AT[j][dst]=A[i][j];
        }
    }
    flag = equal_single_check(P, AT);
